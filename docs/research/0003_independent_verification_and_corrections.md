...
1. **Sliding caption below the face** → PR-07, design elements, ADR 0003/0006 caption amendments (driven by `response.output_audio_transcript.delta`/`.done`; optional user captions via `session.audio.input.transcription` with `gpt-live-transcribe`, $0.017/min; the device trims its own caption on truncation because the provider does not send a truncated transcript back).

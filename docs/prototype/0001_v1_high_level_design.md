...
5. Assistant transcript deltas produce a sliding caption directly below the face. The laptop proves only the browser's fixed-speed visual queue and interruption behavior; V1-B must replace that heuristic with rendered-audio pacing on the K128 and pass PR-07's identifier, timing, playback-cursor, continuous left-edge completion exit, interruption-clear, heard-boundary-accounting, and legibility checks before PR-07 may be claimed.
...
`prompt/mochi-realtime.ftl` begins with `You are a companion` and contains explicit sections for the canonical pager-expression configuration, user context, reconstructed past conversation history, other retrieved/search context, and device/session context. The trusted `${pager_expression_config_json}` value lists every supported emotion and movement plus the 4,000 ms local cadence and plan limit from `config/pager-expression.js`. The gateway renderer accepts only the six known `${name}` scalar placeholders, performs one non-recursive substitution pass, serializes dynamic records as bounded escaped JSON, rejects unknown schemas, unsupported syntax, and several recognizable credential shapes, sanitizes search URL userinfo/query/fragment data, and caps the final instructions at 24 KiB. This is a deliberately constrained FreeMarker-compatible subset, not a full FreeMarker runtime. It is defense in depth, not a comprehensive secret classifier; the production context selector/redactor must enforce MP-06 and finalization before rendering.

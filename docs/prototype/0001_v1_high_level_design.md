...
During eligible idle time, an injectable random source chooses a 3–5 second first delay and 6–12 second repeat delays. Both pupil-free ivory eyes then move together for 1.6–2.4 seconds: look up, look down, scan side to side, or scan clockwise/counterclockwise. The gesture always returns to center before the next pause. A separate low-amplitude animation runs continuously and changes cadence by emotion. Speech, response generation, connection/fault state, low battery, hidden-page state, or reduced-motion preference cancels large random gaze immediately; reduced motion disables both layers. Animation ownership is layered so transforms do not fight: the eye rig owns conversation activity, a whole-eye carrier owns gaze, another carrier owns emotional “alive” motion, and the ivory aperture owns blinking and geometry. The model calls the constrained `set_pager_emotion` function before speech; its allowlisted value expires locally and never alters the listening indicator or appears in spoken/caption text.

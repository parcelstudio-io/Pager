...
| Private idle | Conversation-button light off; every 5–9 seconds the pupils may look up, roll around, or look down, then settle | Capture gate + local expression director |
...
| Low battery | While resting, eyes dim, open less, and look down; random idle rolls stop, while active-conversation movement remains available | PMIC/fuel gauge through power manager |

Expression arbitration is local and layered rather than one winner-takes-all state. Conversation activity owns rig movement, battery capacity owns energy/openness while charging adds an independent glow, expression geometry selects fault/low-capacity before validated affect before an activity default before mood, and active conversation gaze outranks idle curiosity. A constrained future `set_expression` tool can choose only an allowlisted cue and expiry—it cannot provide CSS, control a status light, or claim that Mochi literally feels an emotion. Random accents run only while eligible idle, stop for low power and reduced-motion preference, and never change amber/cyan/off truth. This avoids a cheerful face when the network has failed or a curious roll that implies listening. Power-off is dark by definition—there is no software "muted" face; private idle with the button light off is the quiet state. See [ADR 0010](../decisions/0010_use_local_hierarchical_expression_arbitration.md).

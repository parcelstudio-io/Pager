...
Expression arbitration is local and layered rather than one winner-takes-all state. Conversation activity owns rig movement, battery capacity owns energy/openness while charging remains an independent reported fact, expression geometry selects fault/low-capacity before validated affect before an activity default before mood, and active conversation gaze outranks idle curiosity. The solid ivory eyes never glow; state and character come from shape, opacity, motion, and gaze. The ivory apertures always have a low-amplitude emotion-specific alive rhythm. Eligible large idle gestures last 1.6–2.4 seconds: both dark pupils make a fast conjugate saccade while both apertures follow toward up, down, either side, any of four screen corners, or a clockwise/counterclockwise scan, then always return to center. Before speaking, the constrained `set_pager_emotion` Realtime function can choose one of 28 allowlisted cues plus up to six allowlisted large gaze commands. The prompt, tool schema, and renderer share `config/pager-expression.js`; the device starts planned commands four seconds apart and cancels them for user speech, low power, hidden/reduced-motion state, or loss of the live session. The model cannot provide CSS, control a status light, or claim that Mochi literally feels an emotion. Its metadata never becomes speech or caption text. Random accents stop for low power and reduced-motion preference and never change amber/cyan/off truth. Cosmetic expression code is not a control dependency: import or initialization failure leaves a calm static face, and later animation faults still cannot remove conversation Start/Stop or capture indication. This avoids a cheerful face when the network has failed or a curious scan that implies listening. Power-off is dark by definition—there is no software "muted" face; private idle with the button light off is the quiet state. See [ADR 0010](../decisions/0010_use_local_hierarchical_expression_arbitration.md).

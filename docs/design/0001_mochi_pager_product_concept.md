...
| User speech | Round eyes widen and pupils turn attentively inward | Local VAD |
...
| Delighted | Round eyes brighten, pupils grow, and the pair briefly bounces | Validated, expiring assistant accent |
...
Expression arbitration is local and layered rather than one winner-takes-all state. Conversation activity owns rig movement, battery capacity owns energy/openness while charging adds an independent glow, expression geometry selects fault/low-capacity before validated affect before an activity default before mood, and active conversation gaze outranks idle curiosity. Eligible idle gestures last 1.6–2.4 seconds: both pupils look up or down together, look side to side together, or trace a clockwise/counterclockwise roll, then always return to center. A constrained future `set_expression` tool can choose only an allowlisted cue and expiry—it cannot provide CSS, control a status light, or claim that Mochi literally feels an emotion. Random accents stop for low power and reduced-motion preference and never change amber/cyan/off truth. Cosmetic expression code is not a control dependency: import or initialization failure leaves a calm static face, and later animation faults still cannot remove conversation Start/Stop or capture indication. This avoids a cheerful face when the network has failed or a curious roll that implies listening. Power-off is dark by definition—there is no software "muted" face; private idle with the button light off is the quiet state. See [ADR 0010](../decisions/0010_use_local_hierarchical_expression_arbitration.md).

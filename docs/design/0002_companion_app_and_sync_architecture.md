...
history_context_consent # separate permission to reuse retained history in prompts
memory_consent         # structured facts/summaries; separate from history
updated_by             # account, device, or policy
```

App and pager writes use `client_mutation_id`, the current `binding_generation`, and `If-Match: config_revision`. The gateway either commits the next revision or returns a conflict with the current document; client timestamps never decide winners. Claim, transfer, or recovery creates a fresh binding document with history retention, retained-history context, and structured-memory consent off, and rejects every write from an older generation. The pager clears prior binding-scoped preferences, applies the new defaults, and acknowledges each later revision. A touchscreen volume change may take effect locally while offline, but remains visibly pending with its original base revision; on reconnect it follows the same mutation path, and a conflict resolves to the gateway document with an explicit UI notice. Consent changes require a committed server acknowledgement before taking effect. Safety and capture state are never settings: neither app nor touchscreen can remotely or indirectly start a listening session.
...
Retention is not prompt permission. Only a separate committed `history_context_consent` lets the [server-owned prompt assembler](../decisions/0009_use_server_owned_contextual_prompt_assembly.md) select a bounded finalized history prefix for a later session. Structured user facts require `memory_consent`; search/retrieval is authorized for its immediate purpose. Every selection is scoped to the authenticated `(account_id, device_id, binding_generation)`. Deletion, release/rebinding, account change, or consent withdrawal removes source records and derived summaries, indexes, retrieval results, and prompt caches from future eligibility. Raw audio, deltas, unheard output, and secrets never enter the rendered context, which is provider-processed and therefore covered by separate disclosure.

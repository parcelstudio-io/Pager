...
Requirement PR-07 adds an owner-mandated sliding caption below the face. The device protocol therefore also carries caption text deltas keyed to the same `response_id`/`output_id` as audio (assistant captions from `response.output_audio_transcript.delta`/`.done`; optional user captions from `conversation.item.input_audio_transcription.delta`/`.completed`, enabled via `session.audio.input.transcription`). Transcript deltas can run ahead of rendered audio, so the device paces or trails the caption against playback. After completed playback, the line retains its leftward direction and pace until fully beyond the left edge before clearing; barge-in clears the display immediately and blocks late deltas from that response. The device still computes the heard boundary for cancellation and any opted-in durable history because provider truncation does not return a truncated transcript. Caption first-render latency and caption/audio drift join the measured metrics.

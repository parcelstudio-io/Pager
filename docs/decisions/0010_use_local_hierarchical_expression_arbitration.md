...
CSS animation ownership is layered: an eye rig owns conversation movement, a gaze carrier owns the larger aperture shift, an alive-motion carrier owns persistent emotion cadence, the ivory eye shape owns geometry and blinking, and each dark pupil owns precise gaze within its aperture. This prevents concurrent animations from replacing each other's transforms. Before each spoken answer, the Realtime `set_pager_emotion` function may feed one emotion plus a bounded ordered movement plan from the shared `config/pager-expression.js` allowlists. The same config is rendered into the prompt and defines the 4,000 ms start-to-start cadence; the device schedules the plan locally so network/model timing cannot distort it. User speech and local safety/accessibility priorities cancel or override the plan. The model cannot provide CSS, capture, power, battery, connectivity, or status values. Function-call output is acknowledged before the model continues speaking, so metadata such as `pager_emotion: happy` and gaze commands are never speech or caption text. The product describes these as expressive cues or moods, not proof that the model literally feels emotion.

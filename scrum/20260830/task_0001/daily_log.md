...
**Offline evidence.** The expanded suite passes 46 automated tests, including injected timer/random expression priorities, cancellation and stale-callback rejection, reduced-motion and low-battery-while-charging suppression, bounded context-retrieval timeout, template/schema/injection/credential/URL checks, and proof that browser SDP cannot override the server-rendered Realtime instructions. Syntax and whitespace checks pass. Live provider/audio acceptance and CoreS3 behavior remain pending and are not claimed by these tests.
